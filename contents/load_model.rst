...
--------------------------
    ...
    코드 내에서 다른 세션의 모델을 가져올 수 있습니다. :ref:`nsml.load<nsml.load()>` 로 다른 세션의 모델을 불러와서 :ref:`nsml.save<nsml.save()>` 로 저장 후, 종료하는 더미 세션을 생성하는 방식을 사용합니다.

    baseline의 예제로 방법을 알아보겠습니다.
    ...
    :ref:`모델리스트<nsml model ls>` 를 확인해 보면 다음과 같이 생성된 모델 목록을 확인할 수 있습니다.
    ...
    baseline 코드에서 다음과 같은 :ref:`nsml.load<nsml.load()>` 함수를 호출하는 3줄의 소스코드를 삽입하면 이전에 학습한 모델을 불러올 수 있습니다.
    ...
    3줄의 소스코드를 추가할 가장 좋은 위치는 아래와 같이 ``Trainmode = True`` 코드 밑이며 ``""" Initiate RMSprop optimizer """`` 주석 위입니다.
    ...
    로그에서 다음 문구 2개를 발견하시면 모델이 정상적으로 load되었음을 확인할 수 있습니다. saved라는 이름의 checkpoint가 생성됩니다. nsml.save() 함수에 'saved'라는 이름을 지정했기 때문입니다.

    .. code-block:: console

        $ nsml logs nsml_team/ir_ph1_v2/48
        ...
        model loaded!
        model saved!
        ...
    ...
    .. code-block:: console
        ...
        $ nsml submit nsml_team/ir_ph1_v2/48 saved
        ........
        Building docker image. It might take for a while
        .............
